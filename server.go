...
func (c *Client) infoHandler(w http.ResponseWriter, r *http.Request) {
	ts := c.TorrentClient.Torrents()
	for _, ff := range ts {
		<-ff.GotInfo()
		...
		info := ff.Info()
		o.Name = info.Name
		o.Link = c.ServeTorrent(ff)
		o.Pid = os.Getpid()
		b, err := json.Marshal(&o)
		if err != nil {
			http.Error(w, "Error marshalling json request", http.StatusNotFound)
		} else {
			w.Write(b)
		}
	}
...
func (c *Client) StartServer() {
	...
	http.HandleFunc("/info", c.infoHandler)
